"use strict";
/**
 * Generates docs/reports/SPRINT_4_1_REPORT.pdf from SPRINT_4_1_REPORT.md
 * Generic markdown renderer using pdfkit — zero external binary dependencies.
 * Run: node docs/reports/generate_sprint4_1_pdf.js
 */
...
const INPUT  = path.join(__dirname, "SPRINT_4_1_REPORT.md");
const OUTPUT = path.join(__dirname, "SPRINT_4_1_REPORT.pdf");
...
function newPage() {
  ...
  doc.font(F.bold).fontSize(8).fillColor(C.white)
     .text("FOREX ENGINE PRO  |  SHADOW OS v2 Migration  |  Sprint 4.1 Report", MARGIN, 11);
...
doc.font(F.bold).fontSize(26).fillColor(C.accent)
   .text("SPRINT 4.1 COMPLETION REPORT", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 10;
doc.font(F.regular).fontSize(14).fillColor(C.midGray)
   .text("SHADOW OS v2 — Production PostgreSQL Persistence", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 30;
doc.save().roundedRect(PAGE_W / 2 - 120, y, 240, 34, 6).fill(C.greenBg).restore();
doc.font(F.bold).fontSize(13).fillColor(C.green)
   .text("✓ COMPLETE — 4/4 TESTS PASS", PAGE_W / 2 - 120, y + 10, { width: 240, align: "center" });
y += 60;
doc.font(F.regular).fontSize(10).fillColor(C.lightGray)
   .text("Date: 2026-07-11    ·    Sprint 4.1    ·    FOREX ENGINE PRO", MARGIN, y, { width: CONTENT_W, align: "center" });
