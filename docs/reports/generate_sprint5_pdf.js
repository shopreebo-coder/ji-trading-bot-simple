"use strict";
/**
 * Generates docs/reports/SPRINT_5_REPORT.pdf from SPRINT_5_REPORT.md
...
const INPUT  = path.join(__dirname, "SPRINT_5_REPORT.md");
const OUTPUT = path.join(__dirname, "SPRINT_5_REPORT.pdf");
...
function newPage() {
  ...
  doc.font(F.bold).fontSize(8).fillColor(C.white)
     .text("FOREX ENGINE PRO  |  SHADOW OS v2 Migration  |  Sprint 5 Report", MARGIN, 11);
...
doc.font(F.bold).fontSize(26).fillColor(C.accent)
   .text("SPRINT 5 COMPLETION REPORT", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 10;
doc.font(F.regular).fontSize(14).fillColor(C.midGray)
   .text("SHADOW OS v2 — Shadow LAB Foundation", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 30;
...
doc.font(F.regular).fontSize(10).fillColor(C.lightGray)
   .text("Date: 2026-07-11    ·    Sprint 5    ·    FOREX ENGINE PRO", MARGIN, y, { width: CONTENT_W, align: "center" });
