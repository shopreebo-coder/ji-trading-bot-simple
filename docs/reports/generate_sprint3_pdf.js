"use strict";
/**
 * Generates docs/reports/SPRINT_3_REPORT.pdf from SPRINT_3_REPORT.md
 * Generic markdown renderer using pdfkit — zero external binary dependencies.
 * Run: node docs/reports/generate_sprint3_pdf.js
 */
...
const INPUT  = path.join(__dirname, "SPRINT_3_REPORT.md");
const OUTPUT = path.join(__dirname, "SPRINT_3_REPORT.pdf");
...
function newPage() {
  ...
  doc.font(F.bold).fontSize(8).fillColor(C.white)
     .text("FOREX ENGINE PRO  |  SHADOW OS v2 Migration  |  Sprint 3 Report", MARGIN, 11);
...
doc.font(F.bold).fontSize(26).fillColor(C.accent)
   .text("SPRINT 3 COMPLETION REPORT", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 10;
doc.font(F.regular).fontSize(14).fillColor(C.midGray)
   .text("SHADOW OS v2 — Memory Foundation: MemoryManager", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 30;
doc.save().roundedRect(PAGE_W / 2 - 110, y, 220, 34, 6).fill(C.greenBg).restore();
doc.font(F.bold).fontSize(13).fillColor(C.green)
   .text("✓ COMPLETE — 396/396 TESTS", PAGE_W / 2 - 110, y + 10, { width: 220, align: "center" });
y += 60;
doc.font(F.regular).fontSize(10).fillColor(C.lightGray)
   .text("Date: 2026-07-07    ·    Sprint 3 of 5    ·    FOREX ENGINE PRO", MARGIN, y, { width: CONTENT_W, align: "center" });
