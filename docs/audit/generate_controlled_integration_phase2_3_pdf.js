...
const INPUT = path.join(__dirname, "FOREX_ENGINE_PRO_CONTROLLED_INTEGRATION_PHASE2_3.md");
const OUTPUT = path.join(__dirname, "FOREX_ENGINE_PRO_CONTROLLED_INTEGRATION_PHASE2_3.pdf");
...
const doc = new PDFDocument({
  ...
  info: {
    ...
    Subject: "Controlled Shadow / Selected / Knowledge / Capital Gate integration",
  },
});
...
function newPage() {
  ...
  doc.font("Helvetica-Bold").fontSize(8).fillColor("#fff")
    .text("FOREX ENGINE PRO  |  CONTROLLED INTEGRATION PHASE 2/3", MARGIN, 10);
...
function heading(text, level) {
  ...
  if (level <= 2) {
    doc.save().moveTo(MARGIN, y).lineTo(PAGE_W - MARGIN, y)
      ...
      .strokeColor(level === 1 ? COLORS.accent : COLORS.border)
      .stroke().restore();
    y += 5;
  }
}
...
function table(rows) {
  ...
  cells.forEach((row, rowIndex) => {
    ...
    const heights = row.map((cell, i) => doc.font(header ? "Helvetica-Bold" : "Helvetica")
      .fontSize(size).heightOfString(cell, { width: (colW[i] || CONTENT_W / cols) - 7 }));
    const height = Math.max(18, ...heights) + 8;
    need(height + 2);
    if (header) doc.save().rect(MARGIN, y, CONTENT_W, height).fill(COLORS.accent).restore();
    if (!header && row.some((cell) => /ALLOW|BLOCK|PASS/.test(cell))) {
...
function titlePage() {
  ...
  doc.font("Helvetica-Bold").fontSize(23).fillColor(COLORS.accent)
    .text("CONTROLLED INTEGRATION", MARGIN, y, { width: CONTENT_W, align: "center" });
  y = doc.y + 8;
  ...
  doc.font("Helvetica").fontSize(11).fillColor(COLORS.muted)
    .text("Phase 2/3 — Shadow A/B/C → Selected → Knowledge → Capital Gate", MARGIN, y, {
  ...
  doc.font("Helvetica-Bold").fontSize(13).fillColor(COLORS.green)
    .text("STATUS: VALIDATED", MARGIN + 70, y + 14, { width: CONTENT_W - 140, align: "center" });
  y += 76;
  paragraph("18 sierpnia 2026", { size: 10, color: COLORS.muted });
  paragraph("Pełna walidacja telemetryczna zakończona sukcesem. Workflow uruchomiony po weryfikacji. Publikowanie pozostaje akcją zatwierdzaną przez użytkownika.", {
    size: 9.5,
    color: COLORS.muted,
  });
  y += 16;
...
for (; i < lines.length; i += 1) {
  ...
  if (headingMatch) {
    heading(headingMatch[2], headingMatch[1].length);
    continue;
  }
