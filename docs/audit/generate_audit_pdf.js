"use strict";
/**
 * Generates docs/audit/FOREX_ENGINE_PRO_ARCHITECTURE_AUDIT_v1.pdf
 * from FOREX_ENGINE_PRO_ARCHITECTURE_AUDIT_v1.md
 * Generic markdown renderer using pdfkit — zero external binary dependencies.
 * Run: node docs/audit/generate_audit_pdf.js
 */
...
const INPUT  = path.join(__dirname, "FOREX_ENGINE_PRO_ARCHITECTURE_AUDIT_v1.md");
const OUTPUT = path.join(__dirname, "FOREX_ENGINE_PRO_ARCHITECTURE_AUDIT_v1.pdf");
...
function newPage() {
  ...
  doc.font(F.bold).fontSize(8).fillColor(C.white)
     .text("FOREX ENGINE PRO  |  Full System Architecture Audit  |  v1.0", MARGIN, 11);
...
function codeBlock(lines) {
  ...
  while (i < lines.length) {
    ...
    let yy = y + padding;
    doc.font(F.mono).fontSize(7).fillColor(C.black);
...
function table(rows) {
  ...
  const widths = maxLens.map(l => Math.max(34, (l / totalLen) * CONTENT_W));
  ...
  function rowHeight(cells, font, size) {
    ...
    cells.forEach((cell, c) => {
      const hh = doc.font(font).fontSize(size)
        .heightOfString(clean(cell || ""), { width: colW[c] - 8 });
      h = Math.max(h, hh);
    });
    return h + 8;
  }
  ...
  function drawRow(cells, opts) {
    ...
    cells.forEach((cell, c) => {
      ...
      if (/✅|PASS|COMPLETE|DONE|Production/.test(txt)) color = opts.header ? opts.color : C.green;
      if (/❌|FAIL/.test(txt) && !/0.*FAIL|FAIL.*0/i.test(txt)) color = C.red;
      doc.font(opts.font).fontSize(opts.size).fillColor(color)
         .text(txt, x + 4, y + 4, { width: colW[c] - 8 });
      x += colW[c];
    });
    doc.save().rect(MARGIN, y, CONTENT_W, h)
  ...
  drawRow(header, { font: F.bold, size: 7.5, color: C.white, bg: C.accent, header: true });
  body.forEach((r, i) => {
...
newPage();
vSpace(110);
doc.font(F.bold).fontSize(25).fillColor(C.accent)
   .text("FULL SYSTEM ARCHITECTURE AUDIT", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 8;
doc.font(F.bold).fontSize(18).fillColor(C.black)
   .text("FOREX ENGINE PRO", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 10;
doc.font(F.regular).fontSize(13).fillColor(C.midGray)
   .text("SHADOW OS v2 — Generated from source code (code is the source of truth)", MARGIN, y, { width: CONTENT_W, align: "center" });
y = doc.y + 28;
doc.save().roundedRect(PAGE_W / 2 - 150, y, 300, 34, 6).fill(C.greenBg).restore();
doc.font(F.bold).fontSize(13).fillColor(C.green)
   .text("VERSION 1.0   ·   OVERALL MATURITY ≈ 72%", PAGE_W / 2 - 150, y + 10, { width: 300, align: "center" });
y += 58;
doc.font(F.regular).fontSize(10).fillColor(C.lightGray)
   .text("Audit date: 2026-07-12    ·    15 sections    ·    19 tables    ·    ~56 endpoints", MARGIN, y, { width: CONTENT_W, align: "center" });
