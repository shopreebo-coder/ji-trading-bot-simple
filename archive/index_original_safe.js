// ===================================
// DO NOT MODIFY
// Railway protected original backup
// Emergency restore version
// Project: Snowball
// ===================================
...
const MAIN_TIMEFRAME =
  process.env.TIMEFRAME || "M5";

const ENTRY_TIMEFRAME = "M1";

const RISK_PERCENT = parseFloat(
  process.env.RISK_PERCENT || "0.01"
);

const MAX_OPEN_TRADES = parseInt(
  process.env.MAX_OPEN_TRADES || "1"
);

const MAX_DAILY_TRADES = parseInt(
...
async function getCandles(
  ...
  try {
    const url =
      `${BASE_URL}/v3/instruments/${symbol}/candles`;
...
async function strategy(symbol) {
  try {
    ...
    // COOLDOWN
    const cooldown = 20 * 60 * 1000; // 20 minut

    if (
      cooldownMap[symbol] &&
      Date.now() - cooldownMap[symbol] <= cooldown
    ) {
...
for (const trade of openTrades) {
  ...
  if (
    ...
    const m1LastFast =
      m1Fast[m1Fast.length - 1];

    const m1LastSlow =
      m1Slow[m1Slow.length - 1];
...
const units =
  calculateUnits(
    ...
    // DEBUG
    console.log(
      `${symbol} EMA DIST -> ${emaDistance.toFixed(
        1
      )}`
    );

    console.log(
      `${symbol} CANDLE STR -> ${candleStrength.toFixed(
        2
      )}`
    );

    console.log(
      `${symbol} M1 FAST -> ${m1LastFast.toFixed(
        5
      )}`
    );
...
ema=${emaDistance > 1}
strength=${candleStrength > 0.08}
m1trend=${m1LastFast > m1LastSlow}
m1candle=${m1Bullish}
m1close=${m1LastClose > m1LastFast}
spread=${(m1LastFast - m1LastSlow) >
(0.2 * pipMultiplier(symbol))}`
);
    // BUY
    if (
      lastFast > lastSlow &&
...
(0.1 * pipMultiplier(symbol))
    ) {
      console.log(
        `MTF BUY CONFIRMED -> ${symbol}`
      );
...
(0.2 * pipMultiplier(symbol))}`
    ...
    // SELL
            ...
            m1LastSlow &&
            m1Bearish &&
            (m1LastSlow - m1LastFast) >
            (0.1 * pipMultiplier(symbol))
        ) {
            console.log(
                `MTF SELL CONFIRMED -> ${symbol}`
            );
