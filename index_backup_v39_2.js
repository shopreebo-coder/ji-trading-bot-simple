  ...
  for (const trade of trades) {
    try {
      ...
      if (pips >= 5) {
        ...
        const targetBE =
          parseFloat(breakEven.toFixed(5));
      ...
      // MAX TIME EXIT — increased from 5 min to 10 min to give trades more time to develop
...
async function strategy(symbol) {
  try {
    ...
    // COOLDOWN — reduced from 20 min to 10 min to avoid blocking too many opportunities
    ...
    // EMA DISTANCE
    const emaDistance =
      Math.abs(lastFast - lastSlow) / pipMultiplier(symbol);
    ...
    const marginPercent =
      (account.marginUsed / account.balance) * 100;
    ...
    // CHECK — single block, thresholds identical to real trade conditions
